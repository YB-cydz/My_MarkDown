...
> 一开始的思路是尝试直接将官方包替代川大的，后来发现**川大的相较于官方的已经做出了相当大的修改**（原生fastlio是采用imu与点云耦合来计算里程计，川大修改后基本只看点云了），因此最终选择修改参数的方法
...
代码细节基本上就是参考文档里面将的那样，下面仅解释一些参数、话题上的细节以及为什么这么做。

- **话题修改**：参考文档中的代码的输入输出的话题的消息类型都是`pointcloud2`类型的，这样的好处是方便调用pcl库进行变换，使得两个点云能融合在一起（通俗的说就是把两个雷达的点云通过平移和旋转能够得到一个在参考系中统一的点云），然而，我们需要将融合后的点云输入到`fast_lio`以获得里程计输出，而`fast_lio`的输入话题（可以在fastlio的yaml修改）的消息类型是mid360官方定义的`livox_ros_driver2/msg/CustomMsg`，因而我们调整代码使得他的输入输出都是这个类型（这个消息类型存在一些`pointcloud2`没有的信息，需要从输入获取）
- **外参调整**：因为之前已经在fastlio实现了单个斜雷达的正置，因此这里我们做融合只需要将其中一个雷达的点云旋转到另一个雷达上面就行了（就是选一个雷达作为坐标原点，另一个雷达的外参设置为他们之间的相对关系包括`roll、pitch、yaw、tx、ty、tz`）。**需要注意的是，这里能直接写死变换关系是因为两个雷达的相对位置是固定的，而之前雷达的斜45度安装只是相对于车的，当车动起来之后，相对于地面的变换是时刻变化的，因此选用重力对齐的方法让程序自己去算旋转矩阵，而不是直接在参数文件中写死**
...
创建`implemention.cpp`粘贴以下代码（实际代码参考NUC里面留下来的代码）
...
public:
    ...
    {
        // 声明参数
        this->declare_parameter<std::string>("cloud1_topic", "/livox/lidar_192_168_1_118");#注意修改为正确的ip
        this->declare_parameter<std::string>("cloud2_topic", "/livox/lidar_192_168_1_110");#注意修改为正确的ip
        this->declare_parameter<std::string>("output_topic", "/livox/lidar");
        #直接输出为/livox/lidar话题，省的修改fastlio参数配置文件
...
- 参考文档中对`livox_ros_driver2`里面`rviz`那个launch文件的修改不需要抄，只需要将`launch_ROS2/msg_MID360_launch.py`里面`multi_topic = 0 #多雷达话题`改为`1`就行
- 还记得我们的重力对齐吗，在双雷达融合部署之后，需要在`fastlio`对应的参数文件中将`imu`输入话题修改为下面说的选为原点的雷达对应的`imu`话题（例如`/livox/imu_192_168_1_118`）
