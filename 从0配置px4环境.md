...
#### 关于配置mavros与mavsdk连接时遇到的问题

- 变更了px4.launch中的udp的ip，但是实际输出并未改变
- 暂时通过重新安装的方式解决
- 目前使用qgc就以type-c为主（先不要改动飞控参数了
- 查看udp包数据流向`sudo tcpdump -i any udp port 14550 -n -vv`(抓取 UDP 端口 14550 的网络数据包)，注意`tcpdump`可能需要安装
